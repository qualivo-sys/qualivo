...
(() => {
  ...
  // src/cdn.ts
  var REACT_URL = "/launch/assets/vendor/react.min.js";
  var REACT_SRI = "sha384-DGyLxAyjq0f9SPpVevD6IgztCFlnMF6oW/XQGmfe+IsZ8TqEiDrcHkMLKI6fiB/Z";
  var REACT_DOM_URL = "/launch/assets/vendor/react-dom.min.js";
  var REACT_DOM_SRI = "sha384-gTGxhz21lVGYNMcdJOyq01Edg0jhn/c22nsx0kyqP0TxaV5WVdsSH1fSDUf5YJj1";
  var BABEL_URL = "/launch/assets/vendor/babel.min.js";
...
/* === qualivo.io/launch: envío del formulario a /api/launch-lead (delegación) === */
(function () {
  var T0 = Date.now();
  var BUSY = false, SENT = false;
  document.addEventListener('click', function (ev) {
    var el = ev.target && ev.target.closest ? ev.target.closest('#lf-send') : null;
    if (!el || BUSY || SENT) return;
    var $ = function (id) { return document.getElementById(id); };
    var box = $('launch-form');
    if (!box) return;
    var nombre = ($('lf-nombre') && $('lf-nombre').value || '').trim();
    var empresa = ($('lf-empresa') && $('lf-empresa').value || '').trim();
    var email = ($('lf-email') && $('lf-email').value || '').trim();
    var idea = ($('lf-idea') && $('lf-idea').value || '').trim();
    var msg = function (t, ok) {
      var e = $('lf-msg');
      if (!e) { e = document.createElement('p'); e.id = 'lf-msg'; box.appendChild(e); }
      e.style.cssText = 'margin:10px 0 0;font-size:13px;color:' + (ok ? '#27BDB1' : '#F4CC38');
      e.textContent = t;
    };
    if (!nombre || !idea || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
      msg('Necesitamos tu nombre, un email válido y dos líneas sobre lo que quieres lanzar.');
      return;
    }
    if (Date.now() - T0 < 3000) { msg('Un segundo…'); return; }
    BUSY = true;
    el.textContent = 'Enviando…';
    fetch('/api/launch-lead', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nombre: nombre, empresa: empresa, email: email, idea: idea, website3: ($('lf-hp') && $('lf-hp').value) || '' })
    }).then(function (r) { return r.json().catch(function () { return {}; }); })
      .then(function (j) {
        if (!j.ok) throw new Error('bad');
        SENT = true;
        box.innerHTML = '<div style="text-align:center;padding:10px 0">' +
          '<p style="font-family:var(--qv-font-display),sans-serif;font-size:24px;font-weight:900;color:#27BDB1;margin:0 0 10px">Recibido, ' + nombre.split(' ')[0].replace(/[<>&]/g, '') + '.</p>' +
          '<p style="color:#9AB0C8;font-size:15px;line-height:1.6;margin:0">En menos de 24 h laborables te respondemos con una primera lectura honesta: si vemos la oportunidad, cómo la atacaríamos. Y si no la vemos, también te lo diremos.</p></div>';
        if (window.va) window.va('event', { name: 'launch_lead' });
        if (window.qvTrack) window.qvTrack('launch_lead');
      })
      .catch(function () {
        BUSY = false;
        el.textContent = 'Cuéntanos tu idea →';
        msg('No se pudo enviar. Inténtalo de nuevo o escríbenos a info@maikelechevarria.com.');
      });
  }, true);
})();
