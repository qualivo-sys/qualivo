// Recibe el formulario de agentforme.io y crea/actualiza el contacto en
  ...
  try {
    const upsertRes = await fetch(GHL_BASE + '/contacts/upsert', {
      ...
      body: JSON.stringify({
        ...
        source: 'agentforme.io — formulario analizar mi empresa',
        tags: ['agentforme-landing', 'origen-agentforme']
      })
    });
    if (!upsertRes.ok) {
    ...
    if (contactId) {
      const nota = [
        'Solicitud de análisis — agentforme.io',
        '',
        'Nombre: ' + nombre,
        'Empresa: ' + empresa,
        'Web: ' + (web || '—'),
...
async function notifyByEmail(lead) {
  ...
  const html =
    ...
    '<h2 style="margin:0 0 4px">Nuevo lead en agentforme.io</h2>' +
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: from,
      to: to,
      subject: '🤖 Lead agentforme.io: ' + lead.nombre + ' · ' + lead.empresa,
      html: html,
      reply_to: lead.email
    })
  });
