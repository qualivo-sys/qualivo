...
R()
R("CAMBIOS ACORDADOS EN LA REUNIÓN · estado")
R("1. Tope de CPC","3,00 → 4,50 €","APLICADO el 21/9 · perdíamos el 80 % de las impresiones por puja, no por presupuesto")
R("2. Modificador de móvil","−70 % → −40 %",f"APLICADO el 21/9 · el móvil trae registros a {n(cpl_mov)} €; escritorio a {n(cpl_esc)} €")
R("3. Fin de campaña","miércoles 23 a las 23:59","APLICADO el 21/9 · fecha de finalización en las 4 campañas")
R("4. Presupuesto de Search",f"81,22 → {round(rest/2/5)*5} €/día",f"PENDIENTE DE DECISIÓN · sin esto se quedan {round(rest-81.22*2)} € de los 2.000 sin invertir")
R()
