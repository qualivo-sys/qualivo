...
R("Datos de Google Ads hasta hoy 21/9 a las 09:00 · evento el jueves 24/9")
R()
R("DÓNDE ESTAMOS")
...
R("Pendiente de invertir", rest, "hoy ya está agotado: quedan el martes 22, el miércoles 23 y la mañana del 24")
R("Ritmo necesario", round(rest/2.5,2), f"con el presupuesto actual de 81,22 €/día nos sobrarían {round(rest-81.22*2.5)} €")
R()
...
R()
R("PROPUESTA PARA LO QUE QUEDA DE CAMPAÑA")
R("1. Presupuesto de Search",f"81,22 → {round(rest/2.5/5)*5} €/día",f"para invertir los {n(rest)} € restantes el martes, el miércoles y la mañana del jueves")
...
R("5. Jueves 24","cortar a las 9:00","cuando arranca el evento el gasto deja de aportar")
R()
R(f"Proyección: {n(rest)} € al coste por registro de los últimos 7 días ({n(cpl7)} €) son unos {round(rest/cpl7)} registros más → cerraríamos en torno a {tot_conv + round(rest/cpl7)}.")
R()
