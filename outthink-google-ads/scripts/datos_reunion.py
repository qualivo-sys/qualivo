...
out["campanas"]=[{"n":r["campaign"]["name"],"st":r["campaign"]["status"],"impr":int(r["metrics"].get("impressions",0)),
  ...
  for r in q("SELECT campaign.name, campaign.status, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions FROM campaign WHERE segments.date BETWEEN '2026-08-31' AND '2026-09-21' AND campaign.status != 'REMOVED'")]
out["dias"]=[{"f":r["segments"]["date"],"impr":int(r["metrics"].get("impressions",0)),"clics":int(r["metrics"].get("clicks",0)),
  ...
  for r in q("SELECT segments.date, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions FROM customer WHERE segments.date BETWEEN '2026-08-31' AND '2026-09-21'")]
...
out["disp"]=[{"f":r["segments"]["date"],"d":r["segments"]["device"],"clics":int(r["metrics"].get("clicks",0)),
  "coste":eur(r["metrics"].get("costMicros")),"conv":float(r["metrics"].get("conversions",0))}
  for r in q("SELECT segments.date, segments.device, metrics.clicks, metrics.cost_micros, metrics.conversions FROM campaign WHERE segments.date BETWEEN '2026-08-31' AND '2026-09-21' AND campaign.name='OT26_Search'")]
out["grupos"]=[{"n":r["adGroup"]["name"],"st":r["adGroup"]["status"],"clics":int(r["metrics"].get("clicks",0)),
  "coste":eur(r["metrics"].get("costMicros")),"conv":float(r["metrics"].get("conversions",0))}
  for r in q("SELECT ad_group.name, ad_group.status, metrics.clicks, metrics.cost_micros, metrics.conversions FROM ad_group WHERE segments.date BETWEEN '2026-08-31' AND '2026-09-21' AND ad_group.status != 'REMOVED' AND campaign.name='OT26_Search'")]
