...
Prioridad, de mayor a menor:
  1. Emergencia  -> heal fuerte / potion
  2. Curacion    -> heal normal
  3. Mana        -> mana potion
  4. Targeting   -> elegir y atacar
"""
...
class RulePolicy:
    ...
    def _maybe_heal(self, s: GameState, cfg: Config, now: float, out: list[Action]) -> bool:
        ...
        if s.mana >= cost and self._ready("spell", now):
            self._arm("spell", now, cfg.spell_cooldown_s)
            out.append(Action.cast(spell))
            return True

        # La potion entra solo si el hechizo no es una opcion (sin mana), no
        # por el mero hecho de estar enfriando: quemar potions con el mana
        # lleno es tirar dinero, y en un hunt de una hora son cientos.
        # En emergencia si alternamos potion y hechizo -- es lo que hace un
        # humano, y es lo que te salva cuando el cooldown no llega a tiempo.
        spell_unavailable = s.mana < cost
        if (emergency or spell_unavailable) \
                and s.item_count(cfg.health_potion) > 0 \
                and self._ready("potion", now):
