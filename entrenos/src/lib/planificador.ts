...
import type { Bloque, DiaPlan, Ejercicio, Nivel, Objetivo, Patron, Perfil, Plan, Rol } from './types';
...
function candidatos(perfil: Perfil, patron: Patron): Ejercicio[] {
...
export function prescripcion(rol: Rol, objetivo: Objetivo, nivel: Nivel, ejercicioId: string): Omit<Bloque, 'ejercicioId' | 'rol'> {
...
function cardio(objetivo: Objetivo): string | null {
  switch (objetivo) {
    case 'perder_grasa':
...
function notas(perfil: Perfil): string[] {
  const base = [
...
export function firmaPerfil(p: Perfil): string {
...
export function planDesactualizado(plan: Plan | null, perfil: Perfil): boolean {
  return !!plan && plan.firma !== firmaPerfil(perfil);
}

/** Series totales por grupo muscular en la semana, para revisar el volumen. */
export function volumenSemanal(plan: Plan): { musculo: string; series: number }[] {
