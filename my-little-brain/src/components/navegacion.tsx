...
import { Activity, Brain, CalendarCheck, Dumbbell, MessageCircle, Wallet } from 'lucide-react';
...
export default function Navegacion() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-border bg-card/95 backdrop-blur">
      <div className="mx-auto grid max-w-2xl grid-cols-6 px-1 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">
        {SECCIONES.map(({ href, etiqueta, Icono }) => {
          ...
          return (
            ...
            >
              <Icono size={19} strokeWidth={activo ? 2.4 : 1.8} />
              {etiqueta}
            </Link>
          );
        })}
