/* @ds-bundle: {"format":4,"namespace":"QualivoDesignSystem_04bbe9","components":[],"sourceHashes":{"deck-stage.js":"e9f4ceb60475","design-canvas.jsx":"862a6db59c7c","render-mp4/render.js":"67506ac4b46a"},"inlinedExternals":[],"unexposedExports":[]} */

(() => {

const __ds_ns = (window.QualivoDesignSystem_04bbe9 = window.QualivoDesignSystem_04bbe9 || {});
...
/*
 * Renderiza los 14 creativos HTL a MP4 1080×1920 con música + ticks.
 * Requisitos: node, ffmpeg en PATH, `npm install`, `npx playwright install chromium`.
 * Uso: `npm run render`  (o `node render.js 3` para renderizar solo el nº 3)
 */

const OUTRO = [650, 970, 1290, 1610, 1930, 2250, 2570, 2890, 3210, 4000, 4600, 6700];
const CREATIVES = [{
  name: '01-chat-fantasma',
  dur: 14000,
  ticks: [1200, 2400, 3800, 5000, 6800, 8200]
}, {
  name: '02-carrera-por-el-lead',
  dur: 11000,
  ticks: [400, 2600, 6800, 7800]
}, {
  name: '03-dinero-quemandose',
  dur: 10000,
  ticks: [800]
}, {
  name: '04-la-carta',
  ...
  name: '05-match-en-directo',
  dur: 11000,
  ticks: [600, 1800, 2800, 3400, 4400, 5200]
}, {
  name: '06-subasta-8-segundos',
  dur: 13000,
  ticks: [1100]
}, {
  name: '07-lead-se-enfria',
  dur: 13000,
  ticks: [1000]
}, {
  name: '08-tragaperras',
  ...
  name: '09-lluvia-de-leads',
  dur: 13000,
  ticks: [700, 1420, 2140, 2860, 3580, 4300, 5400, 6800]
}, {
  name: '10-poligrafo',
  dur: 12000,
  ticks: [500, 1200, 4400, 5600]
}, {
  name: '11-speedtest',
  dur: 12000,
  ticks: [800, 5800, 6600]
}, {
  name: '12-bateria-del-interes',
  dur: 13000,
  ticks: [1000]
}, {
  name: '13-que-incluye',
  dur: 13000,
  ticks: [1300, 1760, 2220, 2680, 3140, 3600, 4060, 4520, 4980, 6100, 7100]
}, {
  name: '14-cta-reserva',
  dur: 12000,
  ticks: [400, 1000, 2200, 3200, 4200, 5400],
  isCta: true
}];
...
(async () => {
  ...
  for (let i = 0; i < CREATIVES.length; i++) {
    ...
    const c = CREATIVES[i];
    // duración total: creativo + outro (el outro arranca 600ms antes del final)
    const totalMs = c.isCta ? c.dur : c.dur - 600 + 11200;
    const T = totalMs / 1000;
    // ticks: los del creativo + los del cierre (desplazados)
    const ticks = c.isCta ? c.ticks : c.ticks.concat(OUTRO.map(t => t + c.dur - 600));
    console.log(`\n🎬 [${i + 1}/14] ${c.name} — ${T.toFixed(1)}s, ${ticks.length} ticks`);
    ...
    const t0 = Date.now();
    await page.goto('file://' + path.resolve('player.html') + '?render=' + i);
    await page.evaluate(() => document.fonts.ready);
    await page.waitForTimeout(400); // margen para el logo
    const offsetS = (Date.now() - t0) / 1000;
    await page.evaluate(() => window.__start());
    await page.waitForTimeout(totalMs + 500);
    const videoPath = await page.video().path();
    await ctx.close();

    // mezcla de audio: música (vol 0.16, fade out) + ticks
    const N = ticks.length;
    let fc = `[1:a]volume=0.16,afade=t=out:st=${(T - 0.9).toFixed(2)}:d=0.85[m];`;
    fc += `[2:a]asplit=${N}` + ticks.map((_, k) => `[s${k}]`).join('') + ';';
    fc += ticks.map((t, k) => `[s${k}]adelay=${t}|${t}[d${k}]`).join(';') + ';';
    fc += '[m]' + ticks.map((_, k) => `[d${k}]`).join('') + `amix=inputs=${N + 1}:normalize=0[a]`;
    execSync(`ffmpeg -y -loglevel error -ss ${offsetS.toFixed(2)} -i "${videoPath}" ` + `-stream_loop -1 -i bgm-suave.mp3 -i tick.wav ` + `-filter_complex "${fc}" -map 0:v -map "[a]" ` + `-c:v libx264 -pix_fmt yuv420p -r 30 -crf 18 -c:a aac -b:a 192k ` + `-t ${T} "out/${c.name}.mp4"`, {
      stdio: 'inherit'
    });
    console.log(`   ✅ out/${c.name}.mp4`);
  }
  await browser.close();
  fs.rmSync('tmp', {
    recursive: true,
  ...
  console.log('\n🏁 Listo. Los MP4 están en la carpeta out/');
})().catch(e => {
  console.error(e);
  process.exit(1);
});
})(); } catch (e) { __ds_ns.__errors.push({ path: "render-mp4/render.js", error: String((e && e.message) || e) }); }
