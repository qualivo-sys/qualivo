...
const fs = require('fs');
(async () => {
  const FPS=25, SEG=parseFloat(process.env.SEG||'40'), dir='/home/user/frames-v4h';
  fs.rmSync(dir,{recursive:true,force:true}); fs.mkdirSync(dir);
  const b=await chromium.launch({executablePath:'/opt/pw-browsers/chromium-1194/chrome-linux/chrome'});
  const p=await b.newPage({viewport:{width:1920,height:1080},deviceScaleFactor:1});
  await p.goto('file://'+__dirname+'/escena-h.html',{waitUntil:'networkidle'});
  ...
  for(const t of muestras){ await p.evaluate(t=>window.setT(t), t); await el.screenshot({path:__dirname+'/muestra-h-'+String(t).replace('.','_')+'.png'}); }
