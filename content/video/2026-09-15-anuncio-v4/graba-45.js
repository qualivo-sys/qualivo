...
const fs = require('fs');
(async () => {
  const FPS=25, SEG=parseFloat(process.env.SEG||'40'), dir='/home/user/frames-v45';
  ...
  await p.goto('file://'+__dirname+'/escena-45.html',{waitUntil:'networkidle'});
  ...
  for(const t of muestras){ await p.evaluate(t=>window.setT(t), t); await el.screenshot({path:__dirname+'/muestra-45-'+String(t).replace('.','_')+'.png'}); }
