...
const fs = require('fs');
(async () => {
  const FPS=25, DUR=48.82, dir='/home/user/frames-sis', MODE=process.env.MODE||'todo';
  const b=await chromium.launch({executablePath:'/opt/pw-browsers/chromium'});
  const p=await b.newPage({viewport:{width:1080,height:1920},deviceScaleFactor:1});
  await p.goto('file://'+__dirname+'/escena.html',{waitUntil:'networkidle'}); await p.waitForTimeout(1200);
  const el=await p.$('#e');
  const muestras=[1.2,4.2,7.5,12.5,17.5,23.5,27.5,33,39,45.5];
  for(const t of muestras){ await p.evaluate(t=>window.setT(t), t); await el.screenshot({path:__dirname+'/muestras/m-'+String(t).replace('.','_')+'.png'}); }
