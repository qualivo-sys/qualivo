# Semana 39 · el contenido, intercalando formatos (22-sep-2026)

> Pedido de Maikel el 21-sep por la noche: «dame el contenido que sacaremos esta
> semana, una propuesta de newsletter y un guion; un día carrusel, otro vídeo,
> otro post, otro vídeo, para ir intercalando; y ahora que tenemos leads hay que
> enviar newsletters». Sustituye la tabla del plan del 21-sep
> (`plan-semana-39-contenido.md`) en lo que se contradigan. El resto de ese plan
> (aprendizajes, decisiones, datos y fuentes) sigue valiendo.
>
> **Pausa de redes vigente.** Todo esto son borradores hasta que Maikel la
> levante por escrito. El blog sigue: el artículo de hoy sí está publicado.

## La semana de un vistazo

| Día | Formato | Pieza | Dato que lleva | CTA | Estado |
|---|---|---|---|---|---|
| Mar 22 | **Post de una imagen** (IG + LinkedIn) | Sin humo · «Lo barato es llamar. Lo caro es saber cuándo colgar» | 24 llamadas, 3 citas, 3 × 118 s con locución (bitácora 21-sep) | Conversación | Imagen lista, sin rótulo de serie desde hoy |
| Mié 23 | **Carrusel** (IG + documento LinkedIn) | «Casi 7×» · contestar en la primera hora | HBR 2011, 2.241 empresas; caso propio del lunes | Guardar + pregunta (conversación) | v3 revisada por el Master Reviewer; falta la decisión de la lámina 5 |
| Mié 23 | **Newsletter** · capítulo 1 | «24 llamadas, 3 citas y dos minutos con una operadora» | Bitácora 21-sep, Notion Llamadas | Diagnóstico | `content/newsletter/2026-09-23.md`, revisada (nota 7, cambios aplicados) |
| Jue 24 | **Vídeo a cámara** (reel + LinkedIn) | «Le pedí a un agente que repasara mi CRM. Encontró 34.500 € parados» | CRM propio, 10-sep: 25 de 30, 34.500 €, 15 entre 57 y 63 días | Recurso: «escribe FUGA» | Guion abajo, revisado (nota 7, cambios aplicados) |
| Jue 24 | Blog (ya publicado hoy) | Presupuestos sin respuesta | Los mismos del vídeo | Diagnóstico | `/blog/presupuestos-sin-respuesta/`, publicado 22-sep |
| Vie 25 | **Post de texto** (LinkedIn) + la misma idea en una imagen (IG) | «Mi agente llamó seis minutos después del plantón» | 18-sep: cita 12:00, llamada 12:06 (59 s), WhatsApp a los 3 min | Conversación | Texto en `plan-semana-39-contenido.md` (jueves) y en la serie por etapa (6 · Reunión) |
| Sáb 26 | **Vídeo corto** (reel) | «Mi sistema le dijo a un cliente que no encajaba. Y luego le llamó» | Filtro de «no invierto» quitado el 21-sep | Conversación | Guion en `plan-semana-39-contenido.md` («Reel para grabar a cámara») |

Reparto de CTA: cuatro de conversación, uno de recurso, dos de diagnóstico
(newsletter y blog). Es 57 / 14 / 29 sobre siete piezas; la regla es 50 / 30 /
20 y el recurso va corto porque solo hay un vídeo largo esta semana. La semana
que viene el recurso entra en el carrusel.

Orden de los formatos: imagen, carrusel, vídeo, texto, vídeo. Ningún formato
dos días seguidos. Los dos vídeos son a cámara, sin producción: móvil, luz de
ventana, un plano, rótulos encima.

## Martes 22 · Sin humo · ficha de la pieza del día

- **Serie:** Sin humo (martes). **Formato:** una imagen 1080 × 1350, fondo tinta,
  tarjeta crema con avatar. `content/infografias/2026-09-21/sin-humo-colgar.png`.
- **Cambio de hoy:** fuera el rótulo «Sin humo · verdades de dueño» de arriba
  (regla de Maikel del 22-sep: sin etiquetas de sección). El titular «Sin humo.»
  aguanta solo.
- **Gancho (primera línea del pie):** Mi comercial IA hizo 24 llamadas el lunes y cerró 3 citas. En 3 se quedó escuchando a una operadora.
- **Pie de foto (Instagram):**

  > Mi comercial IA hizo 24 llamadas el lunes y cerró 3 citas. En 3 se quedó
  > escuchando a una operadora.
  >
  > «Gracias. No cuelgues, por favor.» Y no colgaba. Dos minutos cada vez,
  > tres veces el mismo día. Un comercial nuevo habría hecho lo mismo. La
  > diferencia es que al agente le cambias la regla una vez y no vuelve a
  > pasar. Ese mismo lunes: si habla una locución, espera el pitido, deja diez
  > segundos de recado y cuelga.
  >
  > ¿Quién oye las llamadas de tu equipo? Dímelo abajo.
  >
  > #ventas #pymes #comercialia #seguimientocomercial

- **Texto para LinkedIn (misma imagen):** el pie sin etiquetas, y una línea
  más antes de la pregunta: «Lo barato es llamar. Lo caro es el criterio».
- **Fuente:** bitácora del 21-sep. **CTA:** conversación.
- **Qué se mide:** comentarios con respuesta propia, no «me gusta».

## Jueves 24 · guion del vídeo a cámara (45-60 s)

*La pieza más fuerte de la semana: dato propio sorprendente, contado entero,
con el método regalado. Maikel a cámara, un plano, sin saludo, sin logo. Los
rótulos van en mayúsculas, tipografía Anton, naranja sobre el vídeo, uno cada
vez. CTA de recurso.*

> **(0-4 s)** Le pedí a un agente que repasara mi CRM. Encontró 34.500 euros
> parados.
> *Rótulo: 34.500 € PARADOS*
>
> **(4-13 s)** Treinta oportunidades abiertas. Veinticinco paradas: más días de
> la cuenta en la misma etapa, sin que nadie las moviera. Quince llevaban dos
> meses.
> *Rótulo: 25 DE 30 PARADAS · 15 · DOS MESES*
>
> **(13-21 s)** Y ninguna era culpa de nadie. Eran mías. El seguimiento lo
> llevaba en la cabeza, y en la cabeza caben cinco o seis. Las otras veinte
> dormían.
> *Rótulo: ERAN MÍAS*
>
> **(21-33 s)** El agente recorrió los tratos uno a uno con una regla: cada
> etapa tiene un plazo, y el que lo pasa está parado. Dejó veinticinco tareas
> con fecha, con nombre y con el siguiente paso escrito.
> *Rótulo: 25 TAREAS · FECHA · NOMBRE · SIGUIENTE PASO*
> *Plano de dos segundos: la lista de tareas en el CRM, sin nombres, o el móvil con una tarea abierta.*
>
> **(33-45 s)** Tú puedes hacerlo hoy sin ningún agente. Abre el CRM, filtra
> las oportunidades sin actividad en treinta días y suma el importe. Si el
> número te sorprende, está bien contado.
> *Rótulo: SIN ACTIVIDAD EN 30 DÍAS → SUMA*
>
> **(45-53 s)** Tengo la lista de siete cosas que reviso antes de gastar más.
> Esta es la segunda. Escribe FUGA y te la mando.
> *Rótulo: ESCRIBE «FUGA»*

Unas 145 palabras: entre 48 y 55 segundos a ritmo natural. La primera versión
(187 palabras) la recortó el Master Reviewer el 22-sep; no se graba la larga.
Si un día caen más comentarios «FUGA» de los que Maikel puede contestar en 24
horas, responde en el comentario «te lo mando esta noche» y lo manda esa noche.

- **Pie de foto (reel):** El 10 de septiembre le pedí a un agente que repasara
  mi CRM. Encontró 34.500 € declarados parados en 25 oportunidades sin mover.
  Ninguna era culpa de nadie. Eran mías. Cómo contar los tuyos en diez minutos, en el vídeo. Escribe
  FUGA y te mando la lista de siete cosas que reviso antes de gastar más.
  #ventas #crm #pymes #seguimientocomercial
- **Fuentes:** propuesta de valor V1 §5 y §10, estrategia de contenidos §4,
  `assets/app.js` (30 abiertas, 25 paradas, 34.500 €, 25 tareas, 10-sep).
- **Lo que NO dice:** cuánto se ha recuperado de esos 34.500 €. No hay número
  todavía; cuando lo haya, es el capítulo 2 de la newsletter.

## Plan de envíos y respuestas

| Qué | Cuándo | Quién | Cómo |
|---|---|---|---|
| Newsletter capítulo 1 | Miércoles 23, 8:30 | Maikel | Newsletter nativa de LinkedIn. A los leads sin cita, correo individual desde Gmail entre 10:00 y 12:00 (ver `content/newsletter/2026-09-23.md`) |
| Pieza del día | 9:00 (IG y LinkedIn el mismo día) | Maikel desde su perfil | Texto y imagen listos para pegar; no se programa desde GHL mientras la pausa siga |
| Comentarios | Dos pasadas: a las 2 horas de publicar y a las 20:00 | Maikel (yo dejo la respuesta propuesta en el diario si hay una pregunta difícil) | Responder con una pregunta de vuelta, no con un enlace |
| Quien escribe «FUGA» | En las 24 horas siguientes | Maikel, a mano | Mensaje directo con las siete preguntas de `content/recursos/antes-de-gastar-mas.md` en texto, sin PDF, sin enlace a registro. Y una pregunta: «¿cuál de las siete te ha salido peor?» |
| Quien pregunta por el diagnóstico en comentarios | El mismo día | Maikel | Enlace a qualivo.io/diagnostico en respuesta al comentario, no en el post |

Regla de Maikel (21-sep): nada que parezca envío automatizado por WhatsApp
desde el 663. Nada de lo de arriba sale por WhatsApp.

## Revisión

Cada pieza pasa por el Qualivo Master Reviewer antes de darse por lista. Hoy
han pasado: el artículo del blog (nota 6, nueve críticos aplicados en
caliente), la newsletter (nota 7, cinco críticos aplicados) y el guion del
vídeo (nota 7, cuatro críticos aplicados y recortado a 145 palabras). El carrusel ya tiene
su revisión v2 aplicada en la v3. Los resultados quedan en
`content/borradores/revision-master-reviewer-2026-09-22.md` y en la base de
Notion «Revisión de piezas · contenido».

## Decisiones que necesito de Maikel (por escrito)

1. **Levantar la pausa**, y para qué: solo newsletter, solo LinkedIn, o todo.
   Recomendación: newsletter y LinkedIn desde el miércoles; Instagram cuando
   revisemos la estética juntos.
2. **Lámina 5 del carrusel:** ¿se enseña el WhatsApp firmado «soy Maikel» al
   lado de «un comercial IA»? Recomendación: quitar el WhatsApp y dejar solo la
   tarjeta de la llamada de Raquel.
3. **La línea del coste** (1,65 dólares) en la newsletter y en la pieza del
   martes. Recomendación: sí, siempre pegada al fallo.
4. **Los leads sin cita:** ¿les manda el correo individual con el capítulo?
   Recomendación: sí, uno a uno, desde su Gmail, sin herramienta.
